...
class Cube {
  ...
  init() {
    ...
    // RIMUOVI DAVVERO tutti i figli (non assegnare children = [])
    while (this.object.children.length) this.object.remove(this.object.children[0]);
    // ri-aggiungi il gruppo controls
    this.object.add(this.game.controls.group);

    // scala in base alla size
    ...
    // evita culling su tutto il holder
    this.holder.traverse(node => { if (node.frustumCulled) node.frustumCulled = false; });
  ...
  generateModel() {
    ...
    // Materiale plastica del cubetto (risponde alle luci)
    const plasticMaterial = new THREE.MeshPhongMaterial({
      color: 0x222222,       // aggiornato poi in updateColors
      shininess: 30,
      specular: 0x111111
    });

    // Materiale degli sticker: SEMPRE visibile (niente luci)
    const stickerMaterialBase = new THREE.MeshBasicMaterial({
      color: 0xffffff,       // aggiornato poi in updateColors
    ...
    this.positions.forEach((position, index) => {
      ...
      const pieceCube = pieceMesh.clone(); // con materiale plastico
      const pieceEdges = [];
      ...
      position.edges.forEach(pos => {
        const edge = new THREE.Mesh(edgeGeometry, stickerMaterialBase.clone());
        const name = ['L','R','D','U','B','F'][pos];
        const distance = pieceSize / 2;

        edge.position.set(
          distance * [-1, 1, 0, 0, 0, 0][pos],
          distance * [0, 0, -1, 1, 0, 0][pos],
          distance * [0, 0, 0, 0, -1, 1][pos]
        );

        edge.rotation.set(
          Math.PI/2 * [0, 0, 1, -1, 0, 0][pos],
          Math.PI/2 * [-1, 1, 0, 0, 2, 0][pos],
          0
        );
  ...
  updateColors(colors) {
    ...
    this.edges.forEach(edge => {
      if (edge.material) edge.material.color.setHex(colors[edge.name]);
    });
  }

// Transition.js — versione classica (no modules), usa window.Tween/Easing ed espone window.Transition
(function () {
  'use strict';
  ...
  class Transition {
    ...
    buttons(show, hide) {
      const buttonTween = (button, showIt) => {
        return new window.Tween({
          target: button.style,
          duration: 300,
          easing: showIt ? window.Easing.Power.Out(2) : window.Easing.Power.In(3),
          from: { opacity: showIt ? 0 : 1 },
          to:   { opacity: showIt ? 1 : 0 },
          onUpdate: tween => {
            const translate = showIt ? 1 - tween.value : tween.value;
            button.style.transform = `translate3d(0, ${translate * 1.5}em, 0)`;
          },
          onComplete: () => (button.style.pointerEvents = showIt ? 'all' : 'none'),
        });
      };

      (hide || []).forEach(
        name => (this.tweens.buttons[name] = buttonTween(this.game.dom.buttons[name], false))
      );

      setTimeout(() => (show || []).forEach(
        name => (this.tweens.buttons[name] = buttonTween(this.game.dom.buttons[name], true))
      ), hide ? 500 : 0);
    }
    ...
    cube(show, theming = false) {
      ...
      this.tweens.cube = new window.Tween({
        duration: show ? 3000 : 1250,
        easing: show ? window.Easing.Elastic.Out(0.8, 0.6) : window.Easing.Back.In(1),
        onUpdate: tween => {
    ...
    float() {
      ...
      this.tweens.float = new window.Tween({
        duration: 1500,
        easing: window.Easing.Sine.InOut(),
        yoyo: true,
        onUpdate: tween => {
    ...
    zoom(play, time) {
      ...
      const easing = window.Easing.Power.InOut((time > 0) ? 2 : 3);

      this.tweens.zoom = new window.Tween({
      ...
      this.tweens.rotate = new window.Tween({
    ...
    elevate(complete) {
      ...
      this.tweens.elevate = new window.Tween({
        target: this.game.cube.object.position,
        duration: complete ? 1500 : 0,
        easing: window.Easing.Power.InOut(3),
    ...
    stats(show) {
      ...
      if (Array.isArray(this.tweens.stats)) {
        this.tweens.stats.forEach(t => { if (t && t.stop) t.stop(); });
      }
      this.tweens.stats = [];
      ...
      const easing = show ? window.Easing.Power.Out(2) : window.Easing.Power.In(3);

      stats.forEach((stat, index) => {
        const delay = index * (show ? 80 : 60);

        this.tweens.stats[++tweenId] = new window.Tween({
          delay,
          duration: 400,
          easing,
          onUpdate: tween => {
    ...
    theming(show) {
      // FIX: usa 'theme' (non 'prefs') così non si mischiano i tween
    ...
    ranges(ranges, type, show) {
      ...
      if (!Array.isArray(this.tweens[type])) this.tweens[type] = [];
      this.tweens[type].forEach(t => { if (t && t.stop) t.stop(); });
      this.tweens[type] = [];
      ...
      ranges.forEach((range, rangeIndex) => {
        ...
        this.tweens[type][++tweenId] = new window.Tween({
          delay,
          duration: 400,
          easing,
          onUpdate: tween => {
      ...
      this.durations[type] = show
        ? ((ranges.length - 1) * 120) + 200 + listMax * 50 + 400
    ...
    title(show) {
      ...
      this.tweens.title[letters.length] = new window.Tween({
        target: note.style,
        easing: window.Easing.Sine.InOut(),
    ...
    flipLetters(type, letters, show) {
      try {
        (this.tweens[type] || []).forEach(t => t && t.stop && t.stop());
      } catch (e) {}
      ...
      letters.forEach((letter, index) => {
        ...
        this.tweens[type][index] = new window.Tween({
          easing: window.Easing.Sine.Out(),
          duration: show ? 800 : 400,
          delay: index * 50,
          onUpdate: tween => {

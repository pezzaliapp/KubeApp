...
(function () {
  ...
  class IconsConverter {
    constructor(options) {
      ...
      // attributo sempre valido su SVG (meglio dell’uso come CSS)
      this.svgTag.setAttribute('preserveAspectRatio', 'xMidYMid meet');
      ...
      if (options.observe) {
        ...
        this.observer = new MutationObserver(() => { this.convertAllIcons(); });
    ...
    convertIcon(icon) {
      // prende il nome del PRIMO attributo del tag, es: <icon trophy></icon> -> "trophy"
      const firstAttr = icon.attributes && icon.attributes[0];
      if (!firstAttr) return;

      const svgData = this.icons[firstAttr.localName];
      if (typeof svgData === 'undefined') return;

      const svg = this.svgTag.cloneNode(true);
      const vb = (svgData.viewbox || '').split(' ');

      if (svgData.viewbox) {
        svg.setAttribute('viewBox', svgData.viewbox);
        // calcolo width em in base al rapporto w/h del viewBox
        if (vb.length === 4) {
    ...
    addStyles() {
      ...
      // CSS semplificato (niente proprietà non valide)
      style.innerHTML = `
        .${this.className} {
  ...
  const Icons = new IconsConverter({
    ...
    convert: true,   // converti subito tutti i <icon ...>
    // styles: true, // se vuoi iniettare lo stile base .icon via <style>
    observe: false   // metti true se aggiungi icone dinamicamente al DOM
  });

  window.IconsConverter = IconsConverter;
  window.Icons = Icons;
})();
